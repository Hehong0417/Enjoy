...
- (NSString *)lh_h5UrlStringAddUserIDAndToken {
    
    NSString *urlStr = self.copy;
    NSMutableDictionary *parameters = [NSMutableDictionary dictionary];
    HJLoginModel *loginModel = [HJUser sharedUser].loginModel;
    if (loginModel) {
        [parameters setObject:loginModel.userId forKey:@"userId"];
...
- (NSString *)lh_subUrlAddParameters:(NSDictionary *)parameters {
    ...
    HJLoginModel *loginModel = [HJUser sharedUser].loginModel;
    if (loginModel) {
        [newParameters setObject:loginModel.userId forKey:@"userId"];
        [newParameters setObject:loginModel.token forKey:@"token"];
    }
    
    return [NSString lh_subUrlString:urlStr appendingParameters:newParameters];
}
