...
+ (void)load{
    ...
    [[self class] aspect_hookSelector:@selector(viewDidLoad) withOptions:AspectPositionBefore usingBlock:^(id<AspectInfo> info) {
        ...
        //导航栏title设置
        UILabel *titleLabel = [UILabel lh_labelWithFrame:CGRectMake(0, 0, 200, 30) text:@"" textColor:APP_COMMON_COLOR font:AppNavigationFont textAlignment:NSTextAlignmentCenter backgroundColor:kClearColor];
        
        if ([controller isKindOfClass:NSClassFromString(@"HJHomePageTVC")]) {
            
            titleLabel.text = @"阳光抗癌";
            controller.navigationItem.titleView = titleLabel;
        }
        if ([controller isKindOfClass:NSClassFromString(@"HJCircleVC")]) {
            
            titleLabel.text = @"";
            controller.navigationItem.titleView = titleLabel;
        }
        if ([controller isKindOfClass:NSClassFromString(@"HJMessageVC")]) {
            
            titleLabel.text = @"消息";
            controller.navigationItem.titleView = titleLabel;
        }
        if ([controller isKindOfClass:NSClassFromString(@"HJMineTVC")]) {
    ...
    [[self class] aspect_hookSelector:@selector(viewWillAppear:) withOptions:AspectPositionAfter usingBlock:^(id<AspectInfo> info, BOOL animated) {
        ...
        if ([NSStringFromClass([controller class]) hasPrefix:@"HJ"] && ([controller isKindOfClass:NSClassFromString(@"HJPersonCenterVC")]||[controller isKindOfClass:NSClassFromString(@"HJLoginVC")]||[controller isKindOfClass:NSClassFromString(@"HJFoodDetailTVC")]||[controller isKindOfClass:NSClassFromString(@"HJSportDetailTVC")]||[controller isKindOfClass:NSClassFromString(@"HJFamilyUserDetailVC")]||[controller isKindOfClass:NSClassFromString(@"HJBodyOfDateVCViewController")])) {
        ...
        if ([controller isKindOfClass:NSClassFromString(@"HJH5CommonVC")]) {
            //
            HJH5CommonVC *h5VC = controller;
            if(h5VC.hiddenNavigation){
